 ...
 *
 *  - **It is not a floating window, and it does not float at all.**
 *    `WindowManager` (`windows.ts`) is for the draggable, persistent panels of
 *    SPEC §3.4; this is a transient drawer that closes the moment it is used
 *    or dismissed. It mounts IN FLOW inside the chain it belongs to — the slot
 *    `mixer.ts` keeps between a chain's header row and its strip
 *    (`0556-mixer-rail.html`) — so opening it pushes the rows below it down
 *    rather than covering them, and the rail's own scroll carries it.
...
/**
 * Open the picker for `chain` in `slot` — the empty block `mixer.ts` keeps
 * directly under that chain's header row — with `anchorEl` (the chain's `＋`)
 * as the click that outside-close must not count as an outside click.
 * A no-op when the studio has no pointed-at zone (nothing to insert into).
 */
export function openInsertPicker(ctx: SurfaceContext, chain: Chain, anchorEl: HTMLElement, slot: HTMLElement): void {
  closeActive();
  ...
  const el = slot.createDiv({ cls: "wadjet-studio-insert", attr: { role: "menu", "aria-label": `New device → ${CHAIN_LABEL[chain]}` } });
